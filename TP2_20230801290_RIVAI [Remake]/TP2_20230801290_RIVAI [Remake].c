...
void sistemSegitiga() {
	double iTinggi, iAlas, iLuas, _SisiA, _SisiB, _SisiC;
    sistem("CLS");
    cetak("\t\033[0m\033[1;31m");
    gerakGaris();
    cetak("\n\t\t\t\t\t\tAplikasi Segitiga\n\t");
    ...
    _SisiA = getValidDouble();
    cetak("\033[0m\033[1;32m\tMasukkan Sisi B : ");
    _SisiB = getValidDouble();
    cetak("\033[0m\033[1;32m\tMasukkan Sisi C : ");
    _SisiC = getValidDouble();
	
    // Routine Cek Untuk Segitiga Siku-Siku
    if ((_SisiA == sqrt(pow(_SisiB, 2) + pow(_SisiC, 2))) || (_SisiB == sqrt(pow(_SisiA, 2) + pow(_SisiC, 2))) || (_SisiC == sqrt(pow(_SisiA, 2) + pow(_SisiB, 2)))) {
    	// Sisi Miring Pada Sisi A
        if ((_SisiA > _SisiB) && (_SisiA > _SisiC)) {
            iLuas = (_SisiB * _SisiC) * 0.5;
            
			cetak("\033[0m\033[1;34m\tMerupakan Segitiga Siku-Siku Dengan Sisi Miring A,");
        	cetak("\n\tDengan Luas Segitiga = %.2lf\n\n\n\033[0m", iLuas);
        } /*Sisi Miring Pada Sisi B*/ else if ((_SisiB > _SisiA) && (_SisiB > _SisiC)) {
        	iLuas = (_SisiA * _SisiC) * 0.5;

            cetak("\033[0m\033[1;34m\tMerupakan Segitiga Siku-Siku Dengan Sisi Miring B,");
        	cetak("\n\tDengan Luas Segitiga = %.2lf\n\n\n\033[0m", iLuas);
        } /*Sisi Miring Pada Sisi C*/ else if ((_SisiC > _SisiA) && (_SisiC > _SisiA)) {
        	iLuas = (_SisiA * _SisiB) * 0.5;

			cetak("\033[0m\033[1;34m\tMerupakan Segitiga Siku-Siku Dengan Sisi Miring C,");
        	cetak("\n\tDengan Luas Segitiga = %.2lf\n\n\n\033[0m", iLuas);
        }
    } /*Routine Cek Untuk Segitga Sama Sisi*/ else if ((_SisiA == _SisiB) && (_SisiA == _SisiC) && (_SisiB == _SisiC)) {
    	iAlas = 0.5 * _SisiC;
        iTinggi = sqrt(pow(_SisiC, 2) - pow(iAlas, 2));
        iLuas = (iAlas * iTinggi) * 2;
        
		cetak("\033[0m\033[1;34m\tSegitiga Ini Adalah Segitiga Sama Sisi");
        cetak("\n\tDengan Luas Segitiga = %.2lf\n\n\n\033[0m",  iLuas);
    } /*Routine Cek Untuk Segitga Sama Kaki*/ else if ((_SisiA == _SisiC) || (_SisiB == _SisiC) || (_SisiA == _SisiB)) {
    	// Menandakan Bahwa Yang Sama Sisi Adalah Sisi A & Sisi C
        if (_SisiA == _SisiC) {
            iLuas = (_SisiA * _SisiC) * 0.5;
            
            cetak("\033[0m\033[1;34m\tSegitiga Ini Adalah Segitiga Sama Kaki Pada Sisi A & Sisi C,");
        	cetak("\n\tLuas Segitiga Sama Kaki = %.2lf\n\n\n\033[0m", iLuas);
        } /*Menandakan Bahwa Yang Sama Sisi Adalah Sisi B & Sisi C*/ else if (_SisiB == _SisiC) {
        	iLuas = (_SisiB * _SisiC) * 0.5;

        	cetak("\033[0m\033[1;34m\tSegitiga Ini Adalah Segitiga Sama Kaki Pada Sisi B & Sisi C,");
			cetak("\n\tLuas Segitiga Sama Kaki = %.2lf\n\n\n\033[0m", iLuas);
        } /*Menandakan Bahwa Yang Sama Sisi Adalah Sisi A & Sisi B*/ else if (_SisiA == _SisiB) {
        	iLuas = (_SisiA * _SisiB) * 0.5;
                 
        	cetak("\033[0m\033[1;34m\tSegitiga Ini Adalah Segitiga Sama Kaki Pada Sisi A & Sisi B,");
        	cetak("\n\tLuas Segitiga Sama Kaki = %.2lf\n\n\n\033[0m", iLuas);
        }
    } /*Routine Cek Untuk Segitiga Sembarang*/ else {
